...
import { useSessionEquipment } from '../features/equipment/hooks/useEquipment';
import { AssignEquipmentModal } from '../features/equipment/components/AssignEquipmentModal';
import { unassignEquipmentFromSession } from '../features/equipment/api/equipmentApi';
...
export default function SessionDetailsPage() {
    ...
    const isAdminOrOrganizer = role === 'admin-sistema' || role === 'organizator' || role.includes('admin');
    ...
    const {
        items: sessionEquipment,
        isLoading: isLoadingEquipment,
        error: equipmentError,
        refresh: refreshEquipment,
    } = useSessionEquipment(sessionId);
    ...
    useEffect(() => {
        ...
        const loadSession = async () => {
            ...
            try {
                const r = await fetch(`/api/speakers/sessions/${sessionId}`, {
            ...
            <div className="conferences-content">
                <div className="session-card" style={{ maxWidth: '900px', margin: '0 auto' }}>
                    ...
                    <div className="session-card-info">
                        ...
                        <div className="session-info-row" style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', flexWrap: 'wrap' }}>
                            <span className="session-info-icon">🛠️</span>
                            <div className="session-info-content" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%' }}>
                                <span className="session-info-label">DODIJELJENA TEHNIČKA OPREMA</span>
                                {isLoadingEquipment ? (
                                    <p className="session-info-value">Učitavanje opreme...</p>
                                ) : equipmentError ? (
                                    <p className="session-info-value">{equipmentError}</p>
                                ) : sessionEquipment.length === 0 ? (
                                    <p className="session-info-value">Nema dodijeljene opreme za ovu sesiju.</p>
                                ) : (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' }}>
                                        {sessionEquipment.map((eq) => (
                                            ...
                                            >
                                                <div>
                                                    <strong style={{ color: '#fff' }}>{eq.name}</strong>{' '}
                                                    <span style={{ color: '#94a3b8', fontSize: '13px' }}>({eq.type})</span>
                                                    <p style={{ color: '#38bdf8', fontSize: '13px', margin: '4px 0 0 0', fontWeight: '600' }}>
                                                        Količina: {eq.quantity}
                                                    </p>
                                                </div>
                                                {isAdminOrOrganizer && (
                                                    <button
                                                        ...
                                                        onClick={async () => {
                                                            try {
                                                                await unassignEquipmentFromSession(sessionId, eq.equipmentId);
                                                                refreshEquipment();
                                                            } catch {
                                                                alert('Greška pri uklanjanju opreme.');
                                                            }
                                                        }}
                                                    >
                                                        Ukloni
