...
| Funkcionalnost | Unit testiranje | Integracijsko | Sistemsko | Prihvatno | UI testiranje | Regresijsko testiranje |
|---|---|---|---|---|---|---|
| Registracija korisnika (US-01) | DA - validacija obaveznih polja, format emaila, jedinstvenost emaila, minimalna dužina lozinke | DA - upis u bazu, provjera duplikata na DB nivou | DA - forma → potvrda na ekranu, blokada duplikata, redirect na login | DA - potvrda za sve korisničke role | DA - prikaz svih polja forme, validacijske poruke uz ispravno polje, redirect na login nakon uspješne registracije | DA - nakon svake izmjene auth modula; smoke test: registracija → prijava |
| Prijava i odjava (US-02, US-03) | DA - autentifikacija kredencijala, hash provjere, generička poruka greške | DA - login API, baza korisnika i zaštita ruta, invalidacija sesije pri odjavi | DA - login → redirect na početnu; logout → blokada zaštićenih ruta, provjera povratka browserom | DA - potvrda za sve korisničke role | DA - forma za prijavu prikazuje ispravne labele; poruka greške bez otkrivanja detalja; dugme za odjavu vidljivo svim prijavljenim korisnicima | DA - obavezan smoke test nakon svake izmjene; provjera da odjava i dalje invalidira sesiju |
| Upravljanje korisničkim profilom (US-04, US-04.1, US-04.2) | DA - format emaila, jedinstvenost, pravila definisanja lozinke, zabrana praznih obaveznih polja | DA - PATCH /profile, API i baza za ažuriranje podataka; potvrda lozinke pri promjeni | DA - korisnik mijenja podatke i dobija potvrdu; blokada duplikat emaila | NE - niži prioritet za UAT | DA - podaci profila prikazani ispravno; polje za lozinku traži potvrdu; poruka uspjeha vidljiva nakon snimanja | DA - nakon izmjena profil modula; provjera da izmjena emaila ne narušava prijavu |
| Korisničke role i permisije (US-05) | DA - provjera svake role (učesnik, organizator, admin), zabrana pristupa van korisničke role | DA - autorizacijski middleware na svakom zaštićenom endpointu | DA - direktan pristup URL-u zaštićene stranice kao neovlašteni korisnik → HTTP 403 | DA - potvrda za sve korisničke role | DA - UI elementi specifični za rolu prikazani/skriveni ispravno (npr. dugme "Kreiraj konferenciju" vidljivo samo organizatoru) | DA - obavezan nakon svake izmjene u sistemu rola; provjera da dodavanje nove role ne narušava postojeće |
| Kreiranje konferencije (US-06) | DA - validacija obaveznih polja (naziv, datum, lokacija), zabrana kreiranja bez organizator role | DA - POST /conferences, upis u bazu, provjera role na API nivou | DA - forma → kreirana konferencija → vidljiva u listi, blokada za učesnike | DA - ključna MVP funkcionalnost | DA - forma prikazuje sva obavezna polja; validacijske poruke uz ispravna polja; potvrda kreiranje vidljiva; forma nedostupna učesniku | DA - obavezan nakon svake izmjene; smoke test: kreiranje konferencije → vidljiva u listi |
| Pregled i pretraga konferencija (US-07, US-08, US-11) | DA - logika filtriranja i pretrage, prikaz prazne liste | DA - GET /conferences, GET /conferences/:id, query parametri pretrage | DA - korisnik vidi sve aktivne konferencije, klik na detalje, prazna lista s porukom, pretraga vraća relevantne rezultate | DA - bitno za otkrivanje konferencija | DA - lista konferencija prikazuje naziv, datum i lokaciju; prazno stanje prikazuje poruku; klik na konferenciju otvara detalje | DA - nakon izmjena modula konferencija; provjera da pretraga i dalje vraća ispravne rezultate |
| Uređivanje i brisanje konferencije (US-09, US-10) | DA - validacija izmijenjenih polja, potvrda brisanja, zabrana neautorizovanih izmjena | DA - PUT/PATCH /conferences/:id, DELETE /conferences/:id, kaskadne provjere | DA - organizator mijenja podatke i klikne na potvrdu, brisanje gdje konferencija nestaje iz liste, blokada za učesnike | DA - kritično za upravljanje sadržajem | DA - forma za uređivanje prikazuje postojeće podatke; dijalog za potvrdu brisanja prikazan; akcije uređivanja/brisanja skrivene od učesnika | DA - nakon izmjena; provjera da brisanje ne prikazuje obrisanu konferenciju nigdje u UI-u |
| Upravljanje sesijama CRUD (US-12.1, US-12.2, US-12.3, US-12.4) | DA - obavezna polja sesije, zabrana duplikata termina u istoj konferenciji, validacija izmjena | DA - CRUD endpointi za sesije, provjera veze sa konferencijom, zabrana konfliktnih termina na DB nivou | DA - kreiranje → sesija vidljiva u rasporedu, brisanje → sesija nestaje iz rasporeda, već postojeći termin → greška | DA - core programska funkcionalnost | DA - lista sesija prikazuje naziv i termin; prazno stanje s porukom; poruka greške za konflikt termina prikazana uz polje | DA - smoke test: kreiranje sesije → vidljiva u rasporedu; regresija nakon izmjena u modulu sesija |
| Dodjela predavača i dvorane sesiji (US-13, US-16) | DA - provjera postojanja predavača/dvorane; zabrana dodjele nepostojećih entiteta | DA - PATCH /sessions/:id/speaker, /room; FK provjere u bazi | DA - dodjela vidljiva u sesiji; pokušaj dodjele nepostojeće dvorane → greška | DA - bitno za kompletnost rasporeda | DA - dropdown za odabir predavača/dvorane prikazuje samo postojeće entitete; dodijeljena dvorana/predavač vidljivi u detaljima sesije | DA - nakon izmjena sesija ili dvorana; provjera da dodjela i dalje funkcioniše |
| Pregled rasporeda konferencije (US-14) | NE | DA - GET /schedule, ispravan redoslijed sesija po terminu | DA - korisnik vidi sve sesije sortirane po vremenu; ne prikazuju se obrisane sesije | DA - planiranje prisustva | DA - raspored prikazuje sesije u kronološkom redoslijedu; obrisane sesije nisu vidljive; prikaz je pregledan i čitljiv | DA - nakon brisanja sesija ili izmjena rasporeda; provjera da raspored ostaje konzistentan |
| Upravljanje dvoranama CRUD (US-15.1, US-15.2, US-15.3, US-15.4) | DA - zabrana naziva dvorane koji je duplikat; blokada brisanja dvorane dodijeljene aktivnoj sesiji | DA - CRUD endpointi za dvorane; referentni integritet pri brisanju | DA - dodavanje → dvorana u listi; brisanje aktivne dvorane → greška s porukom | DA - upravljanje prostorom | DA - lista dvorana prikazuje osnovne podatke; poruka greške za duplikat naziva; dijalog za potvrdu brisanja prikazan; greška za brisanje aktivne dvorane vidljiva u UI-u | DA - nakon izmjena dvorana ili sesija; provjera da brisanje aktivne dvorane ostaje blokirano |
| Prijava učesnika na konferenciju i odjava (US-18, US-19) | DA - zabrana duple prijave; ažuriranje statusa pri odjavi; provjera roka odjave | DA - POST /registrations, DELETE/PATCH; ažuriranje slobodnih mjesta u bazi | DA - prijava → potvrda; duplikat → greška; odjava → ažuriran kapacitet | DA - osnovna funkcionalnost učesnika | DA - dugme "Prijavi se" vidljivo samo neprijavljenim korisnicima; status prijave vidljiv; poruka potvrde prikazana nakon akcije | DA - smoke test: prijava → potvrda → kapacitet ažuriran; regresija nakon izmjena u modulu prijave |
| Prijava na sesiju i kapacitet (US-20, US-21) | DA - zabrana preklapajućih sesija; blokada popunjene sesije; real-time ažuriranje slobodnih mjesta | DA - POST /session-registrations; transakcijska provjera kapaciteta; GET /capacity | DA - istovremena prijava više korisnika → kapacitet se ne prekorači; popunjena sesija → greška | DA - kritično za organizaciju | DA - prikaz slobodnih mjesta ažurira se bez osvježavanja stranice; popunjena sesija prikazuje odgovarajuću poruku ili onemogućeno dugme | DA - nakon izmjena kapaciteta ili prijava; provjera da real-time ažuriranje i dalje funkcioniše |
| Lista učesnika i popunjenost (US-22) | NE | DA - GET /conferences/:id/participants; provjera autorizacije (samo organizator) | DA - organizator vidi listu; učesnik ne može pristupiti; pretraga/filter liste | NE - niži prioritet za UAT | DA - lista vidljiva samo organizatoru; filter i pretraga funkcionišu u UI-u; učesniku nije prikazana opcija za pristup listi | DA - nakon izmjena u modulu prijava; provjera da učesnik i dalje nema pristup |
| Upravljanje kotizacijama (US-23.1, US-23.2) | DA - validacija iznosa; veza kategorije i iznosa; status plaćeno/neplaćeno | DA - CRUD kotizacija; veza sa prijavom učesnika | DA - kreiranje kategorije i iznosa (postaje vidljivo), izmjena (ažurirano) | DA - finansijska funkcionalnost | DA - lista kategorija prikazana ispravno; status plaćeno/neplaćeno vidljiv; forme za kreiranje i izmjenu funkcionišu u UI-u | DA - nakon izmjena kotizacija; provjera da status plaćanja ostaje konzistentan |
| Obavijesti za korisnike (US-24) | DA - okidačka logika za sve predviđene događaje, ispravno primanje obavijesti | DA - integracija servisa za obavijesti sa svim modulima koji okidaju obavijest | DA - prijava na konferenciju → obavijest stigne ispravnom korisniku, pregled historije | DA - korisničko iskustvo | DA - obavijesti su vidljive u UI-u; indikator nepročitanih obavijesti prikazan; historija obavijesti prikazana u pregledu | DA - nakon izmjena u bilo kojem modulu koji okida obavijest; provjera da obavijesti i dalje stižu |
| Materijali za konferenciju i sesije (US-25, US-26) | DA - obavezna polja pri dodavanju; validacija tipa materijala | DA - GET/POST /materials; veza sa konferencijom/sesijom | DA - dodavanje → materijal vidljiv u listi; filter i pretraga materijala | NE - niži prioritet za UAT | DA - lista materijala prikazana za odabranu konferenciju/sesiju; filter i pretraga funkcionišu; klik na materijal otvara detalje | DA - nakon izmjena materijala ili sesija; provjera da materijali ostaju vezani za ispravnu konferenciju/sesiju |
| Logističke aktivnosti CRUD (US-27.1, US-27.2, US-27.3, US-27.4) | DA - validacija obaveznih polja; provjera konfliktnog uređivanja od više organizatora | DA - CRUD /logistics; zabrana konkurentnih izmjena na DB nivou | DA - kreiranje → vidljivo; filter po tipu aktivnosti; brisanje | DA - operativna logistika | DA - lista aktivnosti prikazuje tip i detalje; filter po tipu radi u UI-u; dijalog za potvrdu brisanja prikazan | DA - nakon izmjena logistike ili konferencija; provjera da filter po tipu i dalje radi ispravno |
| Tehnička oprema CRUD + dodjela (US-28.1, US-28.2, US-28.3, US-28.4) | DA - validacija količine; zabrana dodjele više od dostupnog; ažuriranje dostupnog broja | DA - CRUD /equipment; POST /conferences/:id/equipment; provjera dostupnosti u bazi | DA - dodjela → dostupan broj se smanjuje; pokušaj dodjele više od dostupnog → greška | DA - upravljanje resursima | DA - dostupna količina vidljiva u listi; poruka greške prikazana pri prekoračenju količine; dodijeljena oprema vidljiva u detaljima konferencije | DA - nakon izmjena opreme; provjera da dostupna količina ostaje tačna nakon dodjele i brisanja |
| Q&A sesija (US-29.1, US-29.2) | DA - validacija pitanja; provjera da korisnik ima pristup sesiji; označavanje odgovorenih pitanja | DA - POST /qa/questions; GET /qa; veza sa sesijom; real-time prikaz | DA - učesnik šalje pitanje → vidljivo predavaču; predavač označava odgovoreno → uklanja se iz liste | DA - interakcija predavač-učesnik | DA - polje za unos pitanja dostupno samo prisutnim na sesiji; odgovorena pitanja se uklanjaju iz UI liste; real-time prikaz bez osvježavanja | DA - nakon izmjena sesija ili Q&A modula; provjera da real-time prikaz i označavanje odgovorenih funkcionišu |
| Izvještaji za organizatore (US-30) | NE | DA - GET /reports/:conferenceId; agregacija prijava, kapaciteta i kotizacija | DA - organizator vidi statistiku konferencije; preuzimanje izvještaja | DA - povratna informacija organizatoru | DA - izvještaj prikazan sa tačnim podacima u pregledu; dugme za preuzimanje vidljivo i funkcionalno; izvještaj nedostupan učesniku | DA - nakon izmjena prijava ili kotizacija; provjera da podaci u izvještaju ostaju tačni |
